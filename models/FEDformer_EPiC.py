...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # decomp init
        proj = x_enc.view(x_enc.shape[0],-1,2)
        mean = torch.mean(proj, dim=1).unsqueeze(1).repeat(1, self.pred_len, 1)
        # zeros = torch.zeros([x_dec.shape[0], self.pred_len, x_dec.shape[2]],device=x_enc.device)
        # .to(self.device)  # cuda()
        seasonal_init, trend_init = self.decomp(proj) #x_dec[:,:self.label_len,:]
        ...
        trend_init = torch.mean(trend_init, dim=1).unsqueeze(1).repeat(1, self.label_len, 1)
        # print(trend_init.shape,mean.shape)
        # trend_init = torch.add(trend_init,mean)
        seasonal_init = torch.mean(seasonal_init, dim=1).unsqueeze(1).repeat(1, self.label_len, 1)
        # print(seasonal_init.shape,trend_init.shape) 
        ...
        # print(seasonal_part,trend_part)
        # final
...
if __name__ == '__main__':
    ...
    enc_mark = torch.randn([3, configs.seq_len, 1])

    dec = torch.randn([3, configs.label_len+configs.pred_len, 2])
    dec_mark = torch.randn([3, configs.label_len+configs.pred_len, 1])
    out,atten = model.forward(enc, enc_mark, dec, dec_mark)
    print(atten)
