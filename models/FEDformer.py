...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # decomp init
        proj = x_enc.view(x_enc.shape[0],self.label_len,-1)
        mean = torch.mean(x_enc, dim=-1).unsqueeze(-1).repeat(1, 1, 2)
        zeros = torch.zeros([x_dec.shape[0], self.pred_len, x_dec.shape[2]]).to(device)  # cuda()
        seasonal_init, trend_init = self.decomp(proj)
        

        print(seasonal_init.shape,trend_init.shape)
        # decoder input
        trend_init = torch.mean(trend_init, dim=-1).unsqueeze(-1).repeat(1, 1, 2)
        seasonal_init = torch.mean(seasonal_init, dim=-1).unsqueeze(-1).repeat(1, 1, 2)
        print(seasonal_init.shape,trend_init.shape) 
