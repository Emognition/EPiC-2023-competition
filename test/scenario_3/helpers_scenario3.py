...
def save_files(x, y, file_path, phys_folder, ann_folder):
    ...
    subject_num, video_num = map(int, file_name.replace('.csv', '').split('_')[1::2])
...
def load_and_concatenate_train(base_path, sub, split = None):
    ...
    if split == None:
        ...
        for train_test, vids in split.items():
            for vid in vids:
                file_path = os.path.join(base_path, f"sub_{sub}_vid_{vid}.npy")
...
def save_test_data(y_pred, output_folder, y_test_file, test = True, y_test = None):
    ...
    # if test = True. Saves final data for test set
    if test:
        df = pd.read_csv(y_test_file)
        df['valence'] = y_pred[:, 1]
        df['arousal'] = y_pred[:, 0]
        
    # If test false, saves it with the training data, for validation  
    else:
