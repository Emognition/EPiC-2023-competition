...
def run_experiment(top_features=None, use_PCA=False, n_components_PCA = 20, previous_run = False):
    ...
    def test_function(i, top_features=None, use_PCA=False, previous_run = False):
        ...
        return rmse_per_output, importances, X_train.shape[1]
    ...
    with parallel_backend('loky', n_jobs=num_cpu_cores - 5):
        ...
        if use_PCA:
            column_names = [f"PCA{i}" for i in range(results[-1][2])]
        else:
...
def plot_and_save_top_features(all_importances, n_features, plot=True):
...
def rmse_improved(n_features):
    ...
    all_importances_n, df_results_n = run_experiment(top_features=top_features_n, use_PCA=False, n_components_PCA = 20)
